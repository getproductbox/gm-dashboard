...
app.post("/square/sync", async (c) => {
  ...
  // This endpoint is now a thin proxy to the Supabase `sync-and-transform` function.
  // It is intended for manual "Sync now" actions from the UI, not for heavy backfills.
  // Cron-based syncs run fully inside Supabase via `sync-scheduler`.
  const overlap = Number((payload as any).overlap_minutes) >= 0
    ? Number((payload as any).overlap_minutes)
    : 5;
  const maxLookbackDays = Number((payload as any).max_lookback_days) > 0
    ? Number((payload as any).max_lookback_days)
    : 2;
  const dryRun = !!(payload as any).dry_run;
  const locations = Array.isArray((payload as any).locations) && (payload as any).locations.length > 0
    ? (payload as any).locations
    : undefined;
  ...
  const timeoutId = setTimeout(() => controller.abort(), 25000); // keep under Netlify/CF timeout
  ...
  try {
    ...
    try {
      ...
      // leave json as null; we'll still return something
    }

    // Mirror the original behaviour: surface success flag & results if present.
    if (json && typeof json === "object" && !("success" in json)) {
      json = { success: resp.ok, ...json };
    }

    return c.json(json ?? { success: resp.ok }, resp.status as 200);
  } catch (e: any) {
    clearTimeout(timeoutId);
    const message = e?.name === "AbortError"
      ? "sync-and-transform request timed out"
